...
### 延伸應用
...
![](https://www.desfran.com/wp-content/uploads/2020/07/Slide1-1-1024x576.jpg)

LTV（Lifetime Value）指的是顧客終身價值，也就是顧客對企業的貢獻價值，包括顧客在其人生週期中的所有交易金額和利潤。企業可以通過計算顧客的LTV來評估顧客價值，進而針對高價值顧客進行精細化管理。

計算LTV需要掌握以下三個要素：

平均客戶壽命（Average Customer Lifespan）：平均客戶壽命是指一個客戶在你的業務中的平均時間，通常用月數或年數表示。

平均訂單價值（Average Order Value）：平均訂單價值是指一個客戶在每次購買時的平均花費金額。

購買頻率（Purchase Frequency）：購買頻率是指一個客戶在一段時間內購買的次數，通常用月數或年數表示。

下面以一個簡單的例子來說明LTV的計算過程：

假設某網上商城有一位顧客，這位顧客的平均壽命是2年，平均每次購物花費100元，每年購物5次。那麼這位顧客的LTV就是：

LTV = 100元 × 5次 × 2年 = 1000元
